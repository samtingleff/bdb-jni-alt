...
A database handle.
<p>
Database attributes are specified in the {@link com.sleepycat.jni.db.DatabaseConfig DatabaseConfig} class.
<p>
...
<p>
Storage and retrieval are based on key/data pairs; see {@link com.sleepycat.jni.db.DatabaseEntry DatabaseEntry}
...
committed or aborted.  Specifically, this includes {@link com.sleepycat.jni.db.Cursor Cursor} and
{@link com.sleepycat.jni.db.Transaction Transaction} handles.
<p>
Because key/data pairs are cached in memory, failing to sync the file
with the {@link com.sleepycat.jni.db.Database#close Database.close} or {@link com.sleepycat.jni.db.Database#sync Database.sync} methods
may result in inconsistent or lost information.
<p>
When multiple threads are using the {@link com.sleepycat.jni.db.Database Database} handle
...
@throws DatabaseException if a failure occurs.
    ...
    */
    ...
    <p>
    @param key
    The key {@link com.sleepycat.jni.db.DatabaseEntry DatabaseEntry} of the sequence.
    <p>
    @param config
...
@throws DatabaseException if a failure occurs.
    */
    ...
    /**
Return the {@link com.sleepycat.jni.db.Environment Environment} handle for the database environment
    underlying the {@link com.sleepycat.jni.db.Database Database}.
<p>
This method may be called at any time during the life of the application.
<p>
@return
The {@link com.sleepycat.jni.db.Environment Environment} handle for the database environment
    underlying the {@link com.sleepycat.jni.db.Database Database}.
<p>
<p>
@throws DatabaseException if a failure occurs.
...
If a callback function is specified using {@link com.sleepycat.jni.db.DatabaseConfig#setRecordNumberAppender DatabaseConfig.setRecordNumberAppender}, it will be called after the record number has
...
must be specified.
<p>
@param key the key {@link com.sleepycat.jni.db.DatabaseEntry DatabaseEntry} operated on.
<p>
@param data the data {@link com.sleepycat.jni.db.DatabaseEntry DatabaseEntry} stored.
<p>
<p>
<p>
...
must be specified.
    ...
    <p>
    @return
    The method will return {@link com.sleepycat.jni.db.OperationStatus#NOTFOUND OperationStatus.NOTFOUND} if the
    specified key is not found in the database;
        The method will return {@link com.sleepycat.jni.db.OperationStatus#KEYEMPTY OperationStatus.KEYEMPTY} if the
    ...
    deleted;
    otherwise the method will return {@link com.sleepycat.jni.db.OperationStatus#SUCCESS OperationStatus.SUCCESS}.
    <p>
    <p>
@throws DeadlockException if the operation was selected to resolve a
deadlock.
...
@throws DatabaseException if a failure occurs.
    */
    ...
    <p>
    @param key
    The key {@link com.sleepycat.jni.db.DatabaseEntry DatabaseEntry} being compared.
    <p>
    @param txn
For a transactional database, an explicit transaction may be specified to
transaction-protect the operation, or null may be specified to perform the
...
record number, as described in {@link com.sleepycat.jni.db.DatabaseEntry DatabaseEntry}.  This determines
...
that is a subclass of {@link com.sleepycat.jni.db.MultipleEntry MultipleEntry}, otherwise its byte array does not
...
return {@link com.sleepycat.jni.db.OperationStatus#KEYEXIST OperationStatus.KEYEXIST}.
<p>
<p>
@throws DeadlockException if the operation was selected to resolve a
deadlock.
...
{@link com.sleepycat.jni.db.OperationStatus#KEYEXIST OperationStatus.KEYEXIST}.
<p>
<p>
@throws DeadlockException if the operation was selected to resolve a
deadlock.
...
@throws DatabaseException if a failure occurs.
    ...
    */
    ...
    (using the {@link com.sleepycat.jni.db.StatsConfig#setFast StatsConfig.setFast} method), it will access
    ...
    configured using the {@link com.sleepycat.jni.db.StatsConfig#setFast StatsConfig.setFast} method, cached
...
<p>
Applications should never remove databases with open {@link com.sleepycat.jni.db.Database Database}
...
{@link com.sleepycat.jni.db.EnvironmentConfig#addDataDir EnvironmentConfig.addDataDir}, or by setting the "set_data_dir"
string in the database environment's DB_CONFIG file.
<p>
The {@link com.sleepycat.jni.db.Database Database} handle may not be accessed
...
@throws DatabaseException if a failure occurs.
    */
    ...
    {@link com.sleepycat.jni.db.DatabaseConfig#setSortedDuplicates DatabaseConfig.setSortedDuplicates} method informs this
...
or failure.
    ...
    been configured by {@link com.sleepycat.jni.db.VerifyConfig#setOrderCheckOnly VerifyConfig.setOrderCheckOnly}.
    <p>
    @param dumpStream
    ...
    operation has been been configured by {@link com.sleepycat.jni.db.VerifyConfig#setSalvage VerifyConfig.setSalvage}.
    <p>
    ...
    {@link com.sleepycat.jni.db.VerifyConfig#setSalvage VerifyConfig.setSalvage}, all of the key/data pairs in the
