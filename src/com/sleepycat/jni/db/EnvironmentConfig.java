...
import com.sleepycat.jni.db.ReplicationHostAddress;
...
performed using a specified {@link com.sleepycat.jni.db.Environment Environment} handle.
<p>
...
The number of shared memory buffer pools, that is, the number
    ...
    */
    ...
    <p>
    This method only affects the specified {@link com.sleepycat.jni.db.Environment Environment} handle (and
any other library handles opened within the scope of that handle).
For consistent behavior across the environment, all {@link com.sleepycat.jni.db.Environment Environment}
...
True if the Concurrent Data Store applications are configured to
    ...
    */
    ...
    <p>
    Paths specified to {@link com.sleepycat.jni.db.Environment#openDatabase Environment.openDatabase} and
    {@link com.sleepycat.jni.db.Environment#openSecondaryDatabase Environment.openSecondaryDatabase} will be searched
...
{@link com.sleepycat.jni.db.Environment Environment} handle, not an entire database environment.
    <p>
    This method may not be called after the
environment has been opened.
...
on a
per-lock basis by {@link com.sleepycat.jni.db.Environment#lockVector Environment.lockVector}.
<p>
This method configures a database environment, including all threads
of control accessing the database environment, not only the operations
performed using a specified {@link com.sleepycat.jni.db.Environment Environment} handle.
...
True if the system has been configured to to automatically remove log
    ...
    */
    ...
    available, Berkeley DB methods will throw a {@link com.sleepycat.jni.db.DatabaseException DatabaseException}.
...
The maximum number of locked objects.
    */
    ...
    Because {@link com.sleepycat.jni.db.LogSequenceNumber LogSequenceNumber} file offsets are unsigned four-byte
...
True if the system has been configured to copy read-only database files
    ...
    */
    ...
    call Berkeley DB functions, throwing {@link com.sleepycat.jni.db.RunRecoveryException RunRecoveryException}.
    This functionality should never be used for purposes other than
    debugging.
    <p>
    This method only affects the specified {@link com.sleepycat.jni.db.Environment Environment} handle (and
...
True if the database environment is configured to only be accessed
    ...
    */
    ...
    transmitted from a site in a single call to {@link com.sleepycat.jni.db.Environment#processReplicationMessage Environment.processReplicationMessage}.
    <p>
    This method configures a database environment, including all threads
of control accessing the database environment, not only the operations
performed using a specified {@link com.sleepycat.jni.db.Environment Environment} handle.
    ...
    {@link com.sleepycat.jni.db.Environment#processReplicationMessage Environment.processReplicationMessage}.
    */
    ...
    <p>
    @return
    The transmit limit in bytes for a single call to {@link com.sleepycat.jni.db.Environment#processReplicationMessage Environment.processReplicationMessage}.
    */
...
environment has been opened.
    ...
    communication with the server, {@link com.sleepycat.jni.db.DatabaseException DatabaseException}
...
The base segment ID.
    */
    ...
    {@link com.sleepycat.jni.db.EnvironmentConfig#setUseEnvironment EnvironmentConfig.setUseEnvironment} or
    {@link com.sleepycat.jni.db.EnvironmentConfig#setUseEnvironmentRoot EnvironmentConfig.setUseEnvironmentRoot}.
    <p>
...
True if the transactions have been configured to not wait for locks by default.
    */
    ...
    {@link com.sleepycat.jni.db.Environment#checkpoint Environment.checkpoint}.
    <p>
    This method only affects the specified {@link com.sleepycat.jni.db.Environment Environment} handle (and
any other library handles opened within the scope of that handle).
For consistent behavior across the environment, all {@link com.sleepycat.jni.db.Environment Environment}
...
True if the handle is configured to run all transactions at snapshot
    ...
    */
    ...
    transactions is in use, calls to {@link com.sleepycat.jni.db.Environment#beginTransaction Environment.beginTransaction}
...
Timeout values specified for the database environment may be overridden
on a
per-transaction basis by {@link com.sleepycat.jni.db.Transaction#setTxnTimeout Transaction.setTxnTimeout}.
<p>
This method configures a database environment, including all threads
of control accessing the database environment, not only the operations
performed using a specified {@link com.sleepycat.jni.db.Environment Environment} handle.
