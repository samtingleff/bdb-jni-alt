...
public class DatabaseConfig implements Cloneable {
    ...
    */
    ...
    /**
    Configure the {@link com.sleepycat.jni.db.Environment#openDatabase Environment.openDatabase} method to create
    the database if it does not already exist.
    <p>
    @param allowCreate
    If true, configure the {@link com.sleepycat.jni.db.Environment#openDatabase Environment.openDatabase} method to
    ...
    */
    ...
    /**
Return true if the {@link com.sleepycat.jni.db.Environment#openDatabase Environment.openDatabase} method is configured
    to create the database if it does not already exist.
<p>
This method may be called at any time during the life of the application.
<p>
@return
True if the {@link com.sleepycat.jni.db.Environment#openDatabase Environment.openDatabase} method is configured
    to create the database if it does not already exist.
    */
...
the specified {@link com.sleepycat.jni.db.Database Database} handle.
    <p>
...
The number of shared memory buffer pools, that is, the number
    ...
    */
    ...
    it is not.
    <p>
    Calling this method only affects the specified {@link com.sleepycat.jni.db.Database Database} handle
...
True if the database environment is configured to do checksum
    ...
    */
    ...
    {@link com.sleepycat.jni.db.Database Database} handles used to perform read uncommitted or database
...
True if the database is configured to support read uncommitted.
    ...
    */
    ...
    {@link com.sleepycat.jni.db.DatabaseConfig#setSortedDuplicates DatabaseConfig.setSortedDuplicates} is also configured.
    <p>
...
<p>
The {@link com.sleepycat.jni.db.EnvironmentConfig#setErrorHandler EnvironmentConfig.setErrorHandler} and {@link com.sleepycat.jni.db.DatabaseConfig#setErrorHandler DatabaseConfig.setErrorHandler} methods are used to enhance the mechanism for reporting
...
<p>
Alternatively, applications can use {@link com.sleepycat.jni.db.EnvironmentConfig#setErrorStream EnvironmentConfig.setErrorStream} and {@link com.sleepycat.jni.db.DatabaseConfig#setErrorStream DatabaseConfig.setErrorStream} to
...
<p>
For {@link com.sleepycat.jni.db.Database Database} handles opened inside of database environments,
calling this method affects the entire environment and is equivalent to
calling {@link com.sleepycat.jni.db.EnvironmentConfig#setErrorHandler EnvironmentConfig.setErrorHandler}.
<p>
This method may be called at any time during the life of the application.
<p>
@param errorHandler
...
For {@link com.sleepycat.jni.db.Database Database} handles opened inside of database environments,
calling this method affects the entire environment and is equivalent to
calling {@link com.sleepycat.jni.db.EnvironmentConfig#setErrorPrefix EnvironmentConfig.setErrorPrefix}.
<p>
This method may be called at any time during the life of the application.
<p>
@param errorPrefix
...
error, especially during initial application debugging.
<p>
The {@link com.sleepycat.jni.db.EnvironmentConfig#setErrorStream EnvironmentConfig.setErrorStream} and
{@link com.sleepycat.jni.db.DatabaseConfig#setErrorStream DatabaseConfig.setErrorStream} methods are used to enhance
...
{@link com.sleepycat.jni.db.EnvironmentConfig#setErrorPrefix EnvironmentConfig.setErrorPrefix} or {@link com.sleepycat.jni.db.DatabaseConfig#setErrorPrefix DatabaseConfig.setErrorPrefix}), an error string, and a trailing newline character.
<p>
Setting errorStream to null unconfigures the interface.
<p>
Alternatively, applications can use {@link com.sleepycat.jni.db.EnvironmentConfig#setErrorHandler EnvironmentConfig.setErrorHandler} and {@link com.sleepycat.jni.db.DatabaseConfig#setErrorHandler DatabaseConfig.setErrorHandler} to capture
...
The an OutputStream for displaying error messages.
    */
    ...
    /**
    Configure the {@link com.sleepycat.jni.db.Environment#openDatabase Environment.openDatabase} method to fail if
...
{@link com.sleepycat.jni.db.Environment Environment} handle
<p>
This method may be called at any time during the life of the application.
<p>
...
{@link com.sleepycat.jni.db.Database Database} object, not all operations performed on the underlying
database.
    <p>
    This method may not be called after the database is opened.
If the database already exists when it is opened,
...
The database-specific hash function.
    */
    ...
    {@link com.sleepycat.jni.db.DatabaseConfig#setHashFillFactor DatabaseConfig.setHashFillFactor} method must also be called.
...
{@link com.sleepycat.jni.db.DatabaseConfig#setMessageHandler DatabaseConfig.setMessageHandler} methods are used to display
these messages for the application.
<p>
Setting messageHandler to null unconfigures the interface.
<p>
Alternatively, you can use {@link com.sleepycat.jni.db.EnvironmentConfig#setMessageStream EnvironmentConfig.setMessageStream}
and {@link com.sleepycat.jni.db.DatabaseConfig#setMessageStream DatabaseConfig.setMessageStream} to send the additional
information directly to an output streams.  You should not mix these
approaches.
<p>
For {@link com.sleepycat.jni.db.Database Database} handles opened inside of database environments,
calling this method affects the entire environment and is equivalent to
calling {@link com.sleepycat.jni.db.EnvironmentConfig#setMessageHandler EnvironmentConfig.setMessageHandler}.
<p>
This method may be called at any time during the life of the application.
<p>
...
{@link com.sleepycat.jni.db.EnvironmentConfig#setVerboseDeadlock EnvironmentConfig.setVerboseDeadlock} for example.
<p>
The {@link com.sleepycat.jni.db.EnvironmentConfig#setMessageStream EnvironmentConfig.setMessageStream} and
{@link com.sleepycat.jni.db.DatabaseConfig#setMessageStream DatabaseConfig.setMessageStream} methods are used to display
...
<p>
Alternatively, you can use {@link com.sleepycat.jni.db.EnvironmentConfig#setMessageHandler EnvironmentConfig.setMessageHandler}
and {@link com.sleepycat.jni.db.DatabaseConfig#setMessageHandler DatabaseConfig.setMessageHandler} to capture the additional
information in a way that does not use output streams.  You should not
mix these approaches.
<p>
For {@link com.sleepycat.jni.db.Database Database} handles opened inside of database environments,
calling this method affects the entire environment and is equivalent to
calling {@link com.sleepycat.jni.db.EnvironmentConfig#setMessageStream EnvironmentConfig.setMessageStream}.
<p>
This method may be called at any time during the life of the application.
<p>
@param messageStream
...
methods will throw a {@link com.sleepycat.jni.db.RunRecoveryException RunRecoveryException}.  It is often easier
...
called when {@link com.sleepycat.jni.db.RunRecoveryException RunRecoveryException} is about to be thrown from a
Berkeley DB method.
<p>
For {@link com.sleepycat.jni.db.Database Database} handles opened inside of database environments,
calling this method affects the entire environment and is equivalent to
calling {@link com.sleepycat.jni.db.EnvironmentConfig#setPanicHandler EnvironmentConfig.setPanicHandler}.
<p>
This method may be called at any time during the life of the application.
<p>
@param panicHandler
...
The size of the extents used to hold pages in a Queue database,
    ...
    */
    ...
    /**
    Configure {@link com.sleepycat.jni.db.Database#consume Database.consume} to return key/data pairs in
    ...
    <p>
    This flag configures the {@link com.sleepycat.jni.db.Database#consume Database.consume} method to verify
...
(and any other library handles opened within the scope of that handle).
    <p>
    @param queueInOrder
    If true, configure the {@link com.sleepycat.jni.db.Database#consume Database.consume} method to return
...
configuration cannot be cleared.
    */
    ...
    /**
Return true if the {@link com.sleepycat.jni.db.Database#consume Database.consume} method is configured to return
    key/data pairs in order, always returning the key/data item from the
    head of the queue.
<p>
This method may be called at any time during the life of the application.
<p>
@return
True if the {@link com.sleepycat.jni.db.Database#consume Database.consume} method is configured to return
    key/data pairs in order, always returning the key/data item from the
    head of the queue.
    */
...
True if the database is configured in read-only mode.
    */
    ...
    /**
    Configure {@link com.sleepycat.jni.db.Database#append Database.append} to call the function after the
...
The delimiting byte used to mark the end of a record in the
    ...
    */
    ...
    {@link com.sleepycat.jni.db.DatabaseConfig#setRecordPad DatabaseConfig.setRecordPad} for more information).
    <p>
...
The padding character for short, fixed-length records for the
    ...
    */
    ...
    {@link com.sleepycat.jni.db.DatabaseConfig#setRecordDelimiter DatabaseConfig.setRecordDelimiter} method.  For example,
    ...
    underlying database file (for example, the {@link com.sleepycat.jni.db.Database#close Database.close}
    or {@link com.sleepycat.jni.db.Database#sync Database.sync} methods are called), the in-memory copy
    ...
    {@link com.sleepycat.jni.db.DatabaseConfig#setSnapshot DatabaseConfig.setSnapshot} method for more information.
    </b>
    <p>
    <b>
    ...
    are not part of the {@link com.sleepycat.jni.db.Database Database} transaction methodology.  For
    ...
    using either the {@link com.sleepycat.jni.db.Database#sync Database.sync} or {@link com.sleepycat.jni.db.Database#close Database.close}
    methods will fail, of course.  Specify the noSync argument to the
    {@link com.sleepycat.jni.db.Database#close Database.close} method to stop it from attempting to write
...
The name of an underlying flat text database file that is
    ...
    */
    ...
    return of {@link com.sleepycat.jni.db.OperationStatus#KEYEMPTY OperationStatus.KEYEMPTY}.
    <p>
...
True if the database open is enclosed within a transaction.
    ...
    */
    ...
    using {@link com.sleepycat.jni.db.Environment#checkpoint Environment.checkpoint}.
    <p>
